...
public class SList {
  ...
  public void twin() {
    SListNode current = head;
    while (current != null) {
      SListNode duplicateCurrent = new SListNode(current.item);
      duplicateCurrent.next = current.next;
      current.next = duplicateCurrent;
      current = duplicateCurrent.next;
    }
  }
