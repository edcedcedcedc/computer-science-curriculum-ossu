...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    if (horiz > 1) {
      ...
      // Add all vertical interior walls
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
    ...
    DisjointSets sets = new DisjointSets(horiz * vert);
    ...
    for (Wall wall : walls) {
      ...
      if (wall.isHorizontal) {
        // The wall is between (x, y) and (x, y+1)
        if (wall.y + 1 < vert) { // Make sure neighbor is in bounds
          cell1 = wall.x + wall.y * horiz;
          cell2 = wall.x + (wall.y + 1) * horiz;
        } else {
          continue; // Skip this wall
        }
      } else {
        if (wall.x + 1 < horiz) { // Make sure neighbor is in bounds
          cell1 = wall.x + wall.y * horiz;
          cell2 = (wall.x + 1) + wall.y * horiz;
        } else {
          continue; // Skip this wall
        }
      }
    ...
    /**
     * Fill in the rest of this method. You should go through all the walls of the
     * maze in random order, and remove any wall whose removal will not create a
     * cycle. Use the implementation of disjoint sets provided in the set package to
     * avoid creating any cycles.
     *
     * Note the method randInt() further below, which generates a random integer.
     * randInt() generates different numbers every time the program is run, so that
     * you can make lots of different mazes.
     **/
  ...
  /**
   * horizontalWall() determines whether the horizontal wall on the bottom edge of
  ...
  /**
   * verticalWall() determines whether the vertical wall on the right edge of cell
   * (x, y) exists. If the coordinates (x, y) do not correspond to an interior
   * wall, true is returned.
   **/
   ...
   *
   * DO NOT CHANGE THIS METHOD. Your code is expected to work with our copy of
   * this method.
   **/
  protected void diagnose() {
    ...
    outerLoop: for (int j = 0; j < vert; j++) {
      for (int i = 0; i < horiz; i++) {
        if (!cellVisited[i][j]) {
          System.out.println("Not every cell in your maze is reachable from " + "every other cell.");
          mazeFine = false;
          break outerLoop;
        }
      }
   ...
   * this method.
   */
  protected boolean depthFirstSearch(int x, int y, int fromWhere, boolean[][] cellVisited) {
    ...
    if ((fromWhere != FROMRIGHT) && !verticalWall(x, y)) {
      if (cellVisited[x + 1][y]) {
        ...
        cycleDetected = depthFirstSearch(x + 1, y, FROMLEFT, cellVisited) || cycleDetected;
      }
    }
    ...
    if ((fromWhere != FROMBELOW) && !horizontalWall(x, y)) {
      if (cellVisited[x][y + 1]) {
        ...
        cycleDetected = depthFirstSearch(x, y + 1, FROMABOVE, cellVisited) || cycleDetected;
      }
    }
    ...
    if ((fromWhere != FROMLEFT) && !verticalWall(x - 1, y)) {
      if (cellVisited[x - 1][y]) {
        ...
        cycleDetected = depthFirstSearch(x - 1, y, FROMRIGHT, cellVisited) || cycleDetected;
      }
    }
    ...
    if ((fromWhere != FROMABOVE) && !horizontalWall(x, y - 1)) {
      if (cellVisited[x][y - 1]) {
        ...
        cycleDetected = depthFirstSearch(x, y - 1, FROMBELOW, cellVisited) || cycleDetected;
      }
    }
  ...
  /**
   * main() creates a maze of dimensions specified on the command line, prints the
   * maze, and runs the diagnostic method to see if the maze is good.
   */
