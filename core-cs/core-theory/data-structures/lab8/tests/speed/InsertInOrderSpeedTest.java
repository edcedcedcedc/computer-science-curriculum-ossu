...
import static lab8.tests.speed.InsertRandomSpeedTest.waitForPositiveInt;

/**
 * Performs a timing test on three different set implementations. For
 * hashmap.MyHashMap purposes assumes that <K,V> are <String, Integer> pairs.
 * 
 * @author Josh Hug
 * @author Brendan Hu
 */
public class InsertInOrderSpeedTest {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            timeInOrderMap61B(new ULLMap<>(), waitForPositiveInt(input));

            System.out.print("\nEnter # strings to insert into MyHashMap: ");
            timeInOrderMap61B(new MyHashMap<>(), waitForPositiveInt(input));

            System.out.print("\nEnter # strings to insert into Java's HashMap: ");
            timeInOrderHashMap(new HashMap<>(), waitForPositiveInt(input));
    ...
    /**
     * Returns time needed to put N strings into a hashmap.Map61B in increasing
     * order. makes use of speed.StringUtils.nextString(String s)
     */
