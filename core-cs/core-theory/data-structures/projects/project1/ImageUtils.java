...
public class ImageUtils {
  ...
  private static PixImage buffer2PixImage(BufferedImage bImage) {
    ...
    for (int x = 0; x < bImage.getWidth(); x++) {
      for (int y = 0; y < bImage.getHeight(); y++) {
        ...
        pImage.setPixel(x, y, (short) color.getRed(), (short) color.getGreen(), (short) color.getBlue());
      }
    }
    return pImage;
  }
  ...
  static BufferedImage pixImage2buffer(PixImage pImage) {
    BufferedImage bImage = new BufferedImage(pImage.getWidth(), pImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
    for (int x = 0; x < bImage.getWidth(); x++) {
      for (int y = 0; y < bImage.getHeight(); y++) {
        bImage.setRGB(x, y, new Color(pImage.getRed(x, y), pImage.getGreen(x, y), pImage.getBlue(x, y)).getRGB());
      }
    }
    return bImage;
  }
  ...
  private static BufferedImage readTIFF(String filename) {
    return (new RenderedImageAdapter(JAI.create("fileload", filename))).getAsBufferedImage();
  }
  ...
  /**
   * writeTIFF() writes a run-length encoding to a specified file in TIFF format.
   * 
  ...
  /**
   * displayTIFFs displays a sequence of PixImages and pauses until the window is
   * closed.
   * 
