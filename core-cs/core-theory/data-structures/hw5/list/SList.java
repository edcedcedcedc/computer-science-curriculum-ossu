...
public class SList extends List {
   ...
   * null and the SListNode referenced by tail can be reached from the head by a
   * sequence of zero or more "next" references. This implies that the list is not
   * circularly linked. 2) The "size" field is the number of SListNodes that can
   * be accessed from head (including head itself) by a sequence of "next"
   * references. 3) For every SListNode x in an SList l, x.myList = l.
   **/

  /**
   * newNode() calls the SListNode constructor. Use this method to allocate new
   * SListNodes rather than calling the SListNode constructor directly. That way,
   * only this method need be overridden if a subclass of SList wants to use a
  ...
  /**
   * front() returns the node at the front of this SList. If the SList is empty,
