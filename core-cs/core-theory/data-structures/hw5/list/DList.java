...
public class DList extends List {
   ...
   * sentinel node. Note that the sentinel node does not store an item, and is not
   ...
   * x.next != null. 3) For every DListNode x in a DList, x.prev != null. 4) For
   * every DListNode x in a DList, if x.next == y, then y.prev == x. 5) For every
   * DListNode x in a DList, if x.prev == y, then y.next == x. 6) For every DList
   * l, l.head.myList = null. (Note that l.head is the sentinel.) 7) For every
   * DListNode x in a DList l EXCEPT l.head (the sentinel), x.myList = l. 8) size
   * is the number of DListNodes, NOT COUNTING the sentinel, that can be accessed
   * from the sentinel (head) by a sequence of "next" references.
   **/

  /**
   * newNode() calls the DListNode constructor. Use this method to allocate new
   * DListNodes rather than calling the DListNode constructor directly. That way,
   * only this method need be overridden if a subclass of DList wants to use a
   ...
   * return an "invalid" node--a node with the property that any attempt to use it
   * will cause an exception. (The sentinel is "invalid".)
   *
