...
public abstract class ListNode {
  ...
  /**
   * item references the item stored in the current node. myList references the
   * List that contains this node.
   *
  ...
  /**
   * isValidNode returns true if this node is valid; false otherwise. By default,
   * an invalid node is one that doesn't belong to a list (myList is null), but
   * subclasses can override this definition.
   *
  ...
  /**
   * setItem() sets this node's item to "item". If this node is invalid, throws an
   * exception.
   *
   * Performance: runs in O(1) time.
   */
  ...
  /**
   * remove() removes this node from its List. If this node is invalid, throws an
   * exception.
   *
   * @exception InvalidNodeException if this node is not valid.
   */
