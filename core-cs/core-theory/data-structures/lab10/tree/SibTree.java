...
/**
 * The SibTree class implements general trees using linked nodes. Each node has
 * pointers to its parent, its first (leftmost) child, and its sibling to the
 * immediate right.
 * 
...
public class SibTree extends Tree {
  ...
  /**
   * (inherited) size is the number of items in the tree. root is the tree's root
   * node.
   **/
  ...
  /**
   * root() returns the root node, if one exists. Returns an invalid node if the
   * tree is empty.
   */
  ...
  /**
   * insertRoot() inserts a node containing "item" at the root of the tree. If a
   * root already exists, it becomes a child of the new node.
   */
  ...
  public static void main(String[] args) {
    ...
    try {
      if (r != r1.parent()) {
        System.out.println("  ERROR:  parent of root node's child is" + " not the root, but should be.");
      }
      if (r.parent() == null) {
        System.out.println("  ERROR:  parent() returned null.");
      } else if (r.parent().isValidNode()) {
        System.out.println("  ERROR:  parent() of root is valid," + " but shouldn't be.");
      }
    } catch (Exception e) {
    ...
    try {
      ...
      System.out.println("  ERROR:  parent() failed to throw exception on" + " invalid node.");
    } catch (InvalidNodeException e) {
    }
    ...
    try {
      ...
      if (((Integer) r2.item()).intValue() != 12) {
        System.out.println("  ERROR:  Second child of root does not contain" + " the correct key, 12.");
      }
      if (((Integer) r3.item()).intValue() != 13) {
        System.out.println("  ERROR:  Third child of root does not contain" + " the correct key, 13.");
      }
      if (r2.parent() != r) {
        System.out.println("  ERROR:  Second child of root does not think" + " the root is its parent.");
      }
      if (r3.parent() != r) {
        System.out.println("  ERROR:  Third child of root does not think" + " the root is its parent.");
      }
      if (r2.nextSibling() != r3) {
        System.out
            .println("  ERROR:  Second child of root does not think" + " the root's third child is its next sibling.");
      }
      if (r3.nextSibling().isValidNode()) {
        System.out.println("  ERROR:  Third child of root thinks it has" + " a next sibling.");
      }
      ...
      if (((Integer) r31.item()).intValue() != 131) {
        System.out.println("  ERROR:  Node r31 does not contain" + " the correct key, 131.");
      }
      if (((Integer) r32.item()).intValue() != 132) {
        System.out.println("  ERROR:  Node r32 does not contain" + " the correct key, 132.");
      }
      if (r31.parent() != r3) {
        System.out.println("  ERROR:  Node r31 does not think" + " Node r3 is its parent.");
      }
      if (r32.parent() != r3) {
        System.out.println("  ERROR:  Node r32 does not think" + " Node r3 is its parent.");
      }
      if (r31.nextSibling() != r32) {
        System.out.println("  ERROR:  Node r31 does not think" + " Node r32 is its next sibling.");
      }
    ...
    if (t.size() != 6) {
      System.out.println("  ERROR:  tree size is " + t.size() + " but should be 6.");
    }

    System.out.println("The tree looks like this:");
    System.out.print(t.toString());
    System.out.println("[The above sequence should be" + " 1, 11, 12, 13, 131, 132.]");
    ...
    if (t.size() != 5) {
      System.out.println("  ERROR:  tree size is " + t.size() + " but should be 5.");
    }
    try {
      r1 = r.child(1);
    } catch (Exception e) {
    ...
    if (r1 != r2) {
      System.out.println("  ERROR:  after deleting Node r1, Node r2 has" + " not become the first child of the root.");
    }
    ...
    if (t.size() != 4) {
      System.out.println("  ERROR:  tree size is " + t.size() + " but should be 4.");
    }
    try {
      r32 = r3.child(2);
    } catch (Exception e) {
    ...
    try {
      ...
      if (r.child(2) != r3) {
        System.out.println("  ERROR:  removed non-leaf is no longer" + " the root's second child.");
      }
    } catch (Exception e) {
    ...
    System.out.println("The tree looks like this:");
    System.out.print(t.toString());
    System.out.println("[The above sequence should be" + " 1, 12, 13, 131.]");
    ...
    if (t.size() != 0) {
      System.out.println("  ERROR:  tree size is " + t.size() + " but should be zero.");
    }
    r = t.root();
    if (r.isValidNode()) {
