...
import lab6.sortedlist.Keyable;
...
public class AccountData implements Keyable {
  ...
  /**
   * Constructs a new account for customer "newName" with acount number "num" and
  ...
  /**
   * lessThan() returns true if this account's number is less than the argument's
   * account number.
   **/
  ...
  /**
   * getKey() returns this account's account number as the key to use for sorting
   * and comparison.
   **/
  public int getKey() {
    return number;
